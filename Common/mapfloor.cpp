...
MapFloor::MapFloor(const QRectF &sceneRect, QObject *parent) :
    ...
    cCursorCircleR(7.0)
{
    cModesNames.insert(AreaAdd, tr("create area"));
    cModesNames.insert(DoorAdd, tr("create door"));
...
//            break;
    ...
    emit mouseMoved(/*m_cursorCircle->pos()*/);
}
...
{
    rightPos.setX(qMin(qMax(qreal(0.0), pos.x()), sceneRect().right()));
    rightPos.setY(qMin(qMax(qreal(0.0), pos.y()), sceneRect().bottom()));
    return pos == rightPos;
}
...
void MapFloor::finalizeArea()
{
    if (m_tempPolyline.size() > 2)
    {
        ...
        case MapArea::ceIntersection:
            QMessageBox::warning(0, tr("Error while creating area"),
			                     tr("Created area intersects with other areas."
			                        ));
            break;
        case MapArea::ceAreaExist:
			QMessageBox::warning(0, tr("Error while creating area"),
                                 tr("This floor alredy contains the area."));
            break;
        }
    }
    else
        QMessageBox::warning(0, tr("Error while creating area"),
                             tr("Created area has less than 3 vertices."));
}
