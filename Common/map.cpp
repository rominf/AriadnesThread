...
{
    return convertPixToRealM(QSizeF(m_pixSizeX, m_pixSizeY));
}

QSizeF Map::areaSize(const MapArea *area) const
{
    if (area)
        return convertPixToRealM(area->boundingRect().size());
    else
        return QSizeF(0, 0);
}

QPointF Map::pos(const MapFloor *floor) const
{
    return convertPixToRealM(floor->pos());
}
...
QPointF Map::convertPixToDisplayM(const QPointF point) const
{
    return QPointF(convertPixToDisplayM(point.x()),
                   convertPixToDisplayM(point.y()));
}

qreal Map::convertPixToRealM(const qreal r) const
{
    return r/m_pixPerRealM;
}

QPointF Map::convertPixToRealM(const QPointF point) const
{
    return QPointF(convertPixToRealM(point.x()), convertPixToRealM(point.y()));
}

QSizeF Map::convertPixToRealM(const QSizeF size) const
{
    return QSizeF(convertPixToRealM(size.width()),
                  convertPixToRealM(size.height()));
}

QRectF Map::convertPixToRealM(const QRectF rect) const
{
    return QRectF(convertPixToRealM(rect.topLeft()),
                  convertPixToRealM(rect.bottomRight()));
}

qreal Map::convertDisplayMToPix(const qreal r) const
{
    return r*m_pixPerDisplayM;
}

qreal Map::convertRealMToPix(const qreal r) const
{
    return r*m_pixPerRealM;
}
...
bool Map::areaCopy(MapArea *area, int floorFromIndex, int floorToIndex)
{
    ...
    if (floorFromIndex != floorToIndex)
    {
        ...
        case MapArea::ceIntersection:
            QMessageBox::warning(
                    0, tr("Error while copying area"), tr("On the") +
                    floorTo->name() + tr(" floor there is not enough space to "
			                             "insert the area."));
            break;
        case MapArea::ceAreaExist:
            QMessageBox::warning(
                    0, tr("Error while copying area"), tr("") +
                    floorTo->name() + tr(" floor already contains the area."));
            break;
        }
    }
    else
