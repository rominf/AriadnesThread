...
QDataStream & operator<<(QDataStream &output, const Graph &graph)
{
    ...
    for (int i = 0; i < graph.m_nodes.size(); i++)
        if (!nodes.contains(graph.m_nodes.at(i)))
        {
    ...
    output << GraphNode(QPointF(0, 0), 0) << false; // Marking end of nodes
    ...
    // Marking end of nodes
    output << GraphArc::None << GraphArc::Undefined << 0 << 0 << 0.0;
    return output;
}
...
//                node->setVisible(visible);
	...
	GraphNode *node = m_way.at(0);
	if (node)
	{
	    node->setPen(penNode);
	    if (isActive)
		node->setBrush(QBrush(Qt::darkGreen));
	    else
		node->setBrush(brushNode);
	    node->setVisible(visible);
	}
	node = m_way.at(m_way.size() - 1);
        if (node)
        {
            node->setPen(penNode);
            if (isActive)
